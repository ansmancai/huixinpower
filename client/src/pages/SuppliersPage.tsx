  ...
  const loadSuppliers = async () => {
    ...
    try {
      ...
      if (supplierIds.length > 0) {
        const { data: purchases } = await supabase.from('purchases').select('supplier_id, amount').in('supplier_id', supplierIds);
        const { data: payments } = await supabase.from('transactions').select('supplier_id, amount').eq('type', 'payment').in('supplier_id', supplierIds);
        const { data: invoices } = await supabase.from('invoices').select('supplier_id, total_amount').eq('type', 'input').in('supplier_id', supplierIds);
        
        const purchaseMap: Record<string, number> = {};
        const paymentMap: Record<string, number> = {};
        const invoiceMap: Record<string, number> = {};
        
        purchases?.forEach(p => purchaseMap[p.supplier_id] = (purchaseMap[p.supplier_id] || 0) + parseFloat(p.amount));
        payments?.forEach(p => paymentMap[p.supplier_id] = (paymentMap[p.supplier_id] || 0) + Math.abs(parseFloat(p.amount)));
        invoices?.forEach(i => invoiceMap[i.supplier_id] = (invoiceMap[i.supplier_id] || 0) + parseFloat(i.total_amount));
      ...
      if (showUnpaidOnly) {
        filteredData = filteredData.filter(s => (s.purchasedAmount - s.paidAmount) > 0);
      }
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          {canExport && <ExportButton module="suppliers" moduleName="供应商" filter={{ category: category !== 'all' ? category : undefined }} />}
          {canEdit && (
            <>
              ...
              <button onClick={() => navigate('/suppliers/new')} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">+ 新建供应商</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-lg shadow p-4"><p className="text-gray-500 text-sm">已采购总额</p><p className="text-xl font-bold text-blue-600">{formatAmount(summary.totalPurchase)}</p></div>
        <div className="bg-white rounded-lg shadow p-4"><p className="text-gray-500 text-sm">已付款总额</p><p className="text-xl font-bold text-green-600">{formatAmount(summary.totalPaid)}</p></div>
        <div className="bg-white rounded-lg shadow p-4"><p className="text-gray-500 text-sm">已收票总额</p><p className="text-xl font-bold text-orange-600">{formatAmount(summary.totalInvoiced)}</p></div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6 flex flex-wrap gap-4 items-center">
        ...
        <label className="flex items-center gap-2">
          <input type="checkbox" checked={showUnpaidOnly} onChange={(e) => setShowUnpaidOnly(e.target.checked)} className="rounded" />
          <span className="text-sm">只显示有欠款的供应商</span>
        </label>
      </div>
      ...
      {loading ? <div className="text-center py-12">加载中...</div> : (
        <div className="bg-white rounded-lg shadow overflow-x-auto">
          ...
          {suppliers.length === 0 && <div className="text-center py-8 text-gray-500">暂无供应商数据</div>}
        </div>
      )}
      <ImportModal isOpen={showImportModal} onClose={() => setShowImportModal(false)} onSuccess={() => { loadSuppliers(); setShowImportModal(false); }} module="suppliers" moduleName="供应商" />
    </div>
  );
}