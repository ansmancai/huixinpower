...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    requireAuth(request);
    const result = await db.select().from(suppliers).where(eq(suppliers.id, params.id));
    if (!result.length) {
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await db.select().from(suppliers).where(eq(suppliers.id, params.id));
    if (!updated.length) {
      return errorResponse('供应商不存在', 404);
    }
    return successResponse(updated[0], '供应商更新成功');
  } catch (error: any) {
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    await db.delete(suppliers).where(eq(suppliers.id, params.id));
